...
def upload_audio():
    ...
    if file_ext not in allowed_extensions:
        return (
            jsonify(
                {
                    "error": "Invalid file type. Please upload WAV, MP3, M4A, or FLAC files."
                }
            ),
            400,
        )
...
def analyze_text():
    ...
    try:
        ...
        if not transcript or not transcript.strip():
            return (
                jsonify(
                    {"success": False, "error": "Please provide some text to analyze."}
                ),
                400,
            )
        ...
        traceback.print_exc()
        return (
            jsonify({"success": False, "error": f"Error analyzing text: {str(e)}"}),
            500,
        )
